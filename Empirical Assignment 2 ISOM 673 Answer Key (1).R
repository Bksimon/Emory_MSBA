...
library(igraph)
library(stringr)
library(splitstackshape) # for splitting up investor names into separate columns
library(plotly)
library(network)
library(lubridate)
...
# unique id for each deal 
investments[, dealkey := seq_len(.N)] # this is equivalent to seq_len(nrow()) because there are .N rows in the data table

# getting months
investments[, date := as.Date(investments$"Deal Date", "%m/%d/%y")] # helpful guide for potential date formats here https://www.r-bloggers.com/2013/08/date-formats-in-r/

# floor_date() from lubridate library rounds down to the nearest month, pulling out the calendar month of each funding event
...
length(months) # shows from the beginning of the data to the end there are ~400 months


# focus on investors column, maintain month to track network over time
investors = data.table(investors = investments$Investors, dealkey = investments$dealkey, month = investments$month)
...
# if you get warnings about text encoding this should be okay but you can avoid by replacing any non UTF-8 characters with ''
...
investors = t(cSplit(investors, "investors", ",")) # splits out the investors into separate columns

# get all pairs by making each deal into a list element we can use combn() on
...
# much slower, so can be helpful to avoid vectoring on rows with apply functions when possible
...
# can use logical index as in outlook email network setup we went through in class (line 62)
index = sapply(seq_along(possible_pairs), function(i) length(possible_pairs[[i]]) > 3)
possible_pairs = possible_pairs[index]  # leaves only elements with at least two investors

# then combn() to make the endge list, keeping the date and dealkey columns
edges = lapply(seq_along(possible_pairs), function(i) data.table(t(combn(possible_pairs[[i]][-(1:2)], 2)), dealkey = possible_pairs[[i]][1], month = as.Date(possible_pairs[[i]][2], "%Y-%m-%d")))

# converting to data table just so we can refer to column names and make the rest of the code more interpetable
...
colnames(edges) = c("from", "to", "dealkey", "month")

# keeping track of our string splitting from before, we want to account for when commas are included in the name, creating issues with entries like "inc" being included as separate investors
# we're worried mostly about strings of length 3 or 4 for these kinds of issues
unique(c(edges$from[nchar(edges$from) < 5], edges$to[nchar(edges$to) < 5])) # gives one approach to cover most of these cases
...
# setting interarrival/renewal threshold 
setorderv(edges, c("from", "to", "dealkey"))

# can use data table's shift functionality, grouping by investor pair
...
# define tie as decaying if past 90th percentile of interarrival window
edges[, threshold := quantile(interarrival, .90, na.rm = TRUE)]
edges$threshold[1] # around 3 years

edges[is.na(interarrival), interarrival := 0]

# can make sure repeated ties are accounted for as edge attributes for later
setorderv(edges, c("from", "to", "dealkey"))
edges[, repeated_ties := seq_len(.N) - 1, by = c("from","to")]

# just get the edges up to a given month, for each month in the data again using the months indexing object we made before
edges_monthly = lapply(seq_along(months), function(i) edges[month <= months[i]])

# filtering through threshold
edges_monthly = lapply(seq_along(edges_monthly), function(i) edges_monthly[[i]][, tie_age := months[i] - month])

edges_monthly = lapply(seq_along(edges_monthly), function(i) edges_monthly[[i]][tie_age <= threshold,])

# could also have put this into a for loop

# feed these into igraph -- will use the full network but simplified is also okay
vcnets = lapply(seq_along(edges_monthly), function(i) graph.data.frame(edges_monthly[[i]][,c("from","to","repeated_ties")], directed = FALSE))
...
# can also check with the simplified version of the network
...
t.test(mean_kcore$avg_kcore, mean_kcore_simple$avg_kcore) # average k-cores are higher for network that includes all ties, which is what we would expect
...
eigen_yearly = lapply(seq_along(vcnets), function(i) data.table(node = V(vcnets[[i]])$name, eigcent = eigen_centrality(vcnets[[i]])$vector, month = i))
eigen_yearly = rbindlist(eigen_yearly)
setorderv(eigen_yearly, c("month", "eigcent"), c(1,-1)) # have a long column of coreness scores for each month


# calculate concentration as the correlation between the computed coreness scores 
# lapply will return a lilst, and then sapply will return a vector that makes up each element of the list
...
# can save this for later use if the computation takes a while to run
save(cor_eigen, file = "concentration_scores.RData")
load("concentration_scores.RData")

# ideal partition for each month starting from month 14 -- the month where there variation in coreness scores begins
ideal_partitions_cor_eigen = lapply(seq_along(vcnets)[-(1:13)], function(i) data.table(max_concentration = max(cor_eigen[[i]], na.rm = TRUE), ideal_partition = which.max(cor_eigen[[i]]), ideal_partition_prop = which.max(cor_eigen[[i]])/length(cor_eigen[[i]]), month = months[i])) # returns the sie of the core for each month
...
# putting the score into a table
...
conc_plots = lapply(seq_along(vcnets)[-(1:13)][c(seq_along(vcnets)[-(1:13)]%%12 == 2)], function(i)
  ggplot(cor_eigen_dt[month == i]) + geom_point(aes(x=index_prop, y = concentration), size = .5, color = "Grey50") + xlab("Proportion of firms in core partition") + ylab("Concentration of network") + theme_bw() + theme(text=element_text(family="CMU Serif", size=6), axis.text.x = element_text(color = "black"), axis.text.y = element_text(color = "black")) + labs(subtitle = paste0("Year ",(seq_along(vcnets)%/%12 + min(years))[i])) + theme(plot.subtitle = element_text(hjust = 0.5)) + ylim(c(0,1))
  )
# compared to the modular division, the %/% provides integer division to get the correct year on the plot
...
# key insight -- can use variety of measures to talk about core/periphery structure

# develop a plot or descriptive statistics of how many nodes are included in different levels of k-cores
# can look at distribution of coreness to see if it is evenly dispersed or not

# for indexing the network objects
june_1982 = 12*1
june_1986 = 12*5
june_1991 = 12*10
june_1996 = 12*15
...
# can look at the distribution of closness, in the sense that if many firms have very high coreness and some very few firms have very low coreness then that would also suggest that many firms are in the core and then a few are scattered in the periphery
...
# perform a multidimensional scaling on the distance between to see if many firms appear in the center of the plot and there are some other firms scattered, maybe in a ring, etc. outside of this
...
# decompose the network into its largest connected component--the giant component--and see if this component contains most of the firms in the network
...
# can also show development of core/periphery visually using a network plot animation
# can compare early plots to late plots using eigenvector/coreness as a color, for example through an animation like the one from the diffusion example in class and the venture capital animation from the Class 4 slides

# re-compute to retain same order of nodes as is stored by igraph
eigen_yearly = lapply(seq_along(vcnets), function(i) data.table(node = V(vcnets[[i]])$name, eigcent = eigen_centrality(vcnets[[i]])$vector, month = i))
eigen_yearly = rbindlist(eigen_yearly)
...
saveGIF({
  ...
  for(i in seq_along(vcnets)){
    ...
    plot(vcnets[[i]], vertex.size=3, vertex.color=rainbow(length(unique(eigen_yearly[month == i, eigcent])))[eigen_yearly[month == i, as.factor(eigcent)]], vertex.frame.color=rainbow(length(unique(eigen_yearly[month == i, eigcent])))[eigen_yearly[month == i, as.factor(eigcent)]], vertex.label = NA)
  }
...
# use pam() function from cluster library

# just need the average silhouette widths, which can be referred to directly from within the pam() output with $silinfo$avg.width

# generate a vector of average widths for each number of potential clusters and ask which number of clusters provides the highest width
widths = unlist(sapply(seq_len(vcount(vcnets[[june_1996]]))[-vcount(vcnets[[june_1996]])], function(i) pam(dist(get.adjacency(vcnets[[june_1996]], sparse = FALSE)), i)$silinfo$avg.width))

max(widths)
# so 10 clusters provides the highest width, but at .28 it does not pass any of the rules of thumb at .7 or .5

# getting the plot can use
plot(pam(dist(get.adjacency(vcnets[[june_1996]], sparse = FALSE)), 10))
...
getNetStats=function(net)
{
  ...
  eigcent= eigen_centrality(net)$vector # eigen_centrality creates a list object with some other items in it, we just want the actual scores, which are contained in the object called "vector"
  id=V(net)$name
