---
K-Nearest Neighbors Regressor for Home Price Predictor
---
...
The sample code in HW1-Q1-Sample.r does, in fact, follow the instructions rather closely. Thus, the answer True to Q1.5a is acceptable.

However, when we examine the sample sizes of the validation and test data sets, we see that they are not the same size:
```{R}
nrow(TrainData)
nrow(ValData)
nrow(TestData)
```
If they were off by 1 and the total 607+605=1212 was an odd number(it isn't) then the fact that sizes of the validation and test data sets were not the same would be just fine. However, 607+605=1212 is an even number and they are off by 2. Since they are supposed to be the same fraction of the data, it makes sense to make them both 606 observations. So the answer False to Q1.5a with an explanation along the lines I just gave for Q1.5b is also acceptable.
...
```{R}
for(i in 1:length(TrainData)) {
  qqnorm(TrainData[[i]],main=paste("Normal Probability Plot  of",names(TrainData)[i]))
}
```
#Q2.2
...
The best k is in the variable Bestk and the root MSE for the validation data is in RtMSE[Bestk]:
...
TestData$Log.Lot.Area <- TestData$Lot.Area
...
```
The results for the transformed variables are behaving very oddly and are almost certainly junk. I have not explored why this is the case, but I expect that, after transformation, the distance measure is dominated by the number of bedrooms and bathrooms.
...
We now standardize the transformed variables and proceed as before. This standardization will likely remove the odd behavior that we observed for the transformed unstandardized data.
